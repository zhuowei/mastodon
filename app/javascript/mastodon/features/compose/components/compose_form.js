...
class ComposeForm extends ImmutablePureComponent {
  ...
  handleSubmit = () => {
    ...
    // zhuowei: remove length restriction
    if (is_submitting || is_uploading || false /* length(fulltext) > 500 */ || (fulltext.length !== 0 && fulltext.trim().length === 0 && !anyMedia)) {
      return;
    }
  ...
  render () {
    ...
    // zhuowei: remove length restriction
    const disabledButton = disabled || this.props.is_uploading || false /* length(text) > 500 */ || (text.length !== 0 && text.trim().length === 0 && !anyMedia);
    let publishText = '';
